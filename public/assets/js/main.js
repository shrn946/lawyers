;(function($){
    ...
    // Safety wraps for Slick, OwlCarousel, and ModalVideo to prevent TypeError when elements or linked sliders are missing on the active page
    if ($.fn.slick) {
        const originalSlick = $.fn.slick;
        $.fn.slick = function(options) {
            if (this.length === 0) return this;
            // Handle slick commands (like "slickSetOption")
            if (typeof options === "string") {
                return originalSlick.apply(this, arguments);
            }
            if (options && options.asNavFor) {
                const target = options.asNavFor;
                const safeOptions = Object.assign({}, options);
                delete safeOptions.asNavFor;
                // Initialize slick without asNavFor first to avoid race conditions
                const instance = originalSlick.call(this, safeOptions);
                // Defer setting asNavFor until all sliders are initialized
                setTimeout(() => {
                    if ($(target).length > 0) {
                        try {
    ...
    if ($.fn.owlCarousel) {
        const originalOwl = $.fn.owlCarousel;
    ...
    if ($.fn.modalVideo) {
        const originalModal = $.fn.modalVideo;
